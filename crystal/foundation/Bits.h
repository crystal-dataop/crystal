...
inline constexpr unsigned int findLastSet(T const v) {
  ...
  // clang-format off
  using size = std::integral_constant<size_t, std::max(sizeof(T), sizeof(U0))>;
...
CRYSTAL_GEN(8, uint8_t)
#ifdef _MSC_VER
CRYSTAL_GEN(64, _byteswap_uint64)
CRYSTAL_GEN(32, _byteswap_ulong)
CRYSTAL_GEN(16, _byteswap_ushort)
#else
CRYSTAL_GEN(64, __builtin_bswap64)
CRYSTAL_GEN(32, __builtin_bswap32)
CRYSTAL_GEN(16, __builtin_bswap16)
#endif
...
// ntohs, htons == big16
// ntohl, htonl == big32
#define CRYSTAL_GEN1(fn, t, sz) \
  static t fn##sz(t x) { return fn<t>(x); } \

#define CRYSTAL_GEN2(t, sz) \
  CRYSTAL_GEN1(swap, t, sz) \
  CRYSTAL_GEN1(big, t, sz) \
  CRYSTAL_GEN1(little, t, sz)

#define CRYSTAL_GEN(sz) \
  CRYSTAL_GEN2(uint##sz##_t, sz) \
  CRYSTAL_GEN2(int##sz##_t, sz)

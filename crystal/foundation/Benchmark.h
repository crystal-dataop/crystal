...
struct BenchmarkSuspender {
  ...
  template <class F>
  auto dismissing(F f) -> std::invoke_result_t<F> {
    CRYSTAL_SCOPE_EXIT { rehire(); };
    dismiss();
    return f();
  }
...
typename std::enable_if<detail::function_arity<Lambda>::value == 1>::type
 ...
 */
template <typename Lambda>
typename std::enable_if<detail::function_arity<Lambda>::value == 0>::type
addBenchmark(const char* file, const char* name, Lambda&& lambda) {
...
#define BENCHMARK_IMPL(funName, stringName, rv, paramType, paramName)   \
  ...
  static bool CRYSTAL_ANONYMOUS_VARIABLE(crystalBenchmarkUnused) = (    \
    ::crystal::addBenchmark(__FILE__, stringName,                       \
...
#define BENCHMARK(name, ...)                                    \
  BENCHMARK_IMPL(                                               \
    name,                                                       \
    CRYSTAL_STRINGIZE(name),                                    \
    CRYSTAL_ARG_2_OR_1(1, ## __VA_ARGS__),                      \
    CRYSTAL_ONE_OR_NONE(unsigned, ## __VA_ARGS__),              \
    __VA_ARGS__)
...
#define BENCHMARK_NAMED_PARAM(name, param_name, ...)                    \
  BENCHMARK_IMPL(                                                       \
      CRYSTAL_CONCATENATE(name, CRYSTAL_CONCATENATE(_, param_name)),    \
      CRYSTAL_STRINGIZE(name) "(" CRYSTAL_STRINGIZE(param_name) ")",    \
      iters,                                                            \
      unsigned,                                                         \
      iters) {                                                          \
...
#define BENCHMARK_DRAW_LINE()                                                 \
  static bool CRYSTAL_ANONYMOUS_VARIABLE(crystalBenchmarkUnused) =            \
      (::crystal::addBenchmark(__FILE__, "-", []() -> unsigned { return 0; }),\
       true)
...
#define BENCHMARK_SUSPEND                                   \
  if (auto CRYSTAL_ANONYMOUS_VARIABLE(BENCHMARK_SUSPEND) =  \
      ::crystal::BenchmarkSuspender()) {}                   \
  else

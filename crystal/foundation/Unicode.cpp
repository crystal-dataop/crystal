...
char32_t utf8ToCodePoint(
  ...
  if (p >= e) {
    ...
    throw std::runtime_error("crystal::utf8ToCodePoint empty/invalid string");
  }
  ...
  if ((fst & 0xC0) != 0xC0) {
    ...
    throw std::runtime_error(toString("crystal::utf8ToCodePoint i=0 d=", d));
  }
  ...
  for (unsigned int i = 1; i != 3 && p + i < e; ++i) {
    ...
    if ((tmp & 0xC0) != 0x80) {
      ...
      throw std::runtime_error(
        toString("crystal::utf8ToCodePoint i=", i, " tmp=", (uint32_t)tmp));
    }
    ...
    if (!(fst & 0x80)) {
      ...
      if ((d & ~bitMask[i - 1]) == 0) {
        ...
        throw std::runtime_error(
          toString("crystal::utf8ToCodePoint i=", i, " d=", d));
      }
  ...
  throw std::runtime_error(
      "crystal::utf8ToCodePoint encoding length maxed out");
}

 ...
 *     ;
 *
 * Also see crystal/json.h for the serialization and deserialization
 * functions for JSON.
 *
...
#include "crystal/foundation/operators.h"
...
struct dynamic : private operators<dynamic> {
  enum Type {
    NULLT,
    ARRAY,
    BOOL,
 ...
 public:
  ...
  /* implicit */ dynamic(std::string_view val);
   ...
   * hence we explicitly crystalept the reference proxy.
   */
   ...
   * requested type.
   *
   * Note you can only use this to crystaless integral types or strings,
  ...
  /*
   * It is occasionally useful to crystaless a string's internal pointer
 ...
 public:
  ...
  /*
   * For objects or arrays, provides crystaless to sub-fields by index or
   * field name.
   *
  ...
  /*
   * This works for crystaless to both objects and arrays.
   *
  ...
  /*
   * These functions merge two crystal dynamic objects.

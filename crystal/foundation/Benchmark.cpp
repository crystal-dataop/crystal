...
#include "crystal/foundation/Benchmark.h"
...
#define CRYSTAL_GLOBAL_BENCHMARK_BASELINE crystalGlobalBenchmarkBaseline
#define CRYSTAL_STRINGIZE_X2(x) CRYSTAL_STRINGIZE(x)

// Add the global baseline
BENCHMARK(CRYSTAL_GLOBAL_BENCHMARK_BASELINE) {
  asm volatile("");
}

size_t getGlobalBenchmarkBaselineIndex() {
  const char *global =
      CRYSTAL_STRINGIZE_X2(CRYSTAL_GLOBAL_BENCHMARK_BASELINE);
  auto it = std::find_if(
      benchmarks().begin(),
      benchmarks().end(),
  ...
  CRYSTAL_CHECK(it != benchmarks().end());
  return size_t(std::distance(benchmarks().begin(), it));
}

#undef CRYSTAL_STRINGIZE_X2
#undef CRYSTAL_GLOBAL_BENCHMARK_BASELINE
...
void runBenchmarks() {
  CRYSTAL_CHECK(!benchmarks().empty());

  vector<detail::BenchmarkResult> results;
  results.reserve(benchmarks().size() - 1);

  std::unique_ptr<std::regex> bmRegex;
  if (!FLAGS_bm_regex.empty()) {
    bmRegex = std::make_unique<std::regex>(FLAGS_bm_regex);
  }
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (bm.name != "-") { // skip separators
      if (bmRegex && !std::regex_search(bm.name, *bmRegex)) {
        continue;
      }

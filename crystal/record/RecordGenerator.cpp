...
void RecordClassGenerator::generate() {
  for (auto& config : *configs_) {
    if (config.type() == DataType::TUPLE) {
      RecordClassGenerator gen(out_, config.name(), &config.tuple());
      gen.generate();
    }
  }
  append(R"(
class {{ classname }}Record : public RecordBase {
 public:
  {{ classname }}Record(const untyped_tuple::meta& meta, void* buffer)
      : RecordBase(meta, buffer) {}
)", {{"classname", toCamelCase(classname_)}});
...
void RecordFileGenerator::generateRecord() {
  RecordClassGenerator gen(out_, classname_, &config_->fieldConfigs);
  gen.generate();
}

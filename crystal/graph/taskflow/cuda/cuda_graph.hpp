...
inline void cudaGraph::_make_native_graph() {
  ...
  for(auto& node : _nodes) {
    switch(node->_handle.index()) {
      case cudaNode::NOOP:
        std::get<cudaNode::Noop>(node->_handle).work(
          _native_handle, node->_native_handle
        );
      break;
      
      //case cudaNode::HOST:
      //  std::get<cudaNode::Host>(node->_handle).work(
      //    _native_handle, node->_native_handle
      //  );
      //break;

      case cudaNode::MEMSET:
        std::get<cudaNode::Memset>(node->_handle).work(
          _native_handle, node->_native_handle
        );
      break;

      case cudaNode::COPY:
        std::get<cudaNode::Copy>(node->_handle).work(
          _native_handle, node->_native_handle
        );
      break;

      case cudaNode::KERNEL:
        std::get<cudaNode::Kernel>(node->_handle).work(
          _native_handle, node->_native_handle
        );
      break;
    }

...
class cudaFlow {
  ...
  public:
    ...
    */
    template <typename T>
    std::enable_if_t<
      std::is_pod_v<T> && (sizeof(T)==1 || sizeof(T)==2 || sizeof(T)==4), 
      cudaTask
    > 
...
inline cudaTask cudaFlow::noop() {
  auto node = _graph.emplace_back(std::in_place_type_t<cudaNode::Noop>{}, 
...
cudaTask cudaFlow::kernel(
  ...
  auto node = _graph.emplace_back(std::in_place_type_t<cudaNode::Kernel>{}, 
...
cudaTask cudaFlow::copy(T* tgt, const T* src, size_t num) {
  ...
  auto node = _graph.emplace_back(std::in_place_type_t<cudaNode::Copy>{}, 
